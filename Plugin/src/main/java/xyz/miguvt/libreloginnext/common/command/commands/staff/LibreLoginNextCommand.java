...
import static xyz.miguvt.libreloginnext.common.AuthenticLibreLoginNext.DATE_TIME_FORMATTER;
import static xyz.miguvt.libreloginnext.common.AuthenticLibreLoginNext.GSON;
...
@CommandAlias("libreloginnext|librelogin")
public class LibreLoginNextCommand<P> extends StaffCommand<P> {

    public LibreLoginNextCommand(AuthenticLibreLoginNext<P, ?> plugin) {
        super(plugin);
    }
    ...
    public static <P> void enablePremium(P player, User user, AuthenticLibreLoginNext<P, ?> plugin, boolean onlyValid) {
