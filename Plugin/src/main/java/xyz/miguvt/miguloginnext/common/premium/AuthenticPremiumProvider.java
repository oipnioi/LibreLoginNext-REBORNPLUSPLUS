...
package xyz.miguvt.miguloginnext.common.premium;
...
public class AuthenticPremiumProvider implements PremiumProvider {
    ...
    public AuthenticPremiumProvider(AuthenticMiguLoginNext<?, ?> plugin) {
        this.plugin = plugin;
    ...
    private PremiumUser getUserFromAshcon(String name) throws PremiumException {
        try {
            ...
            switch (connection.getResponseCode()) {
                case 200 -> {
                    var data = AuthenticMiguLoginNext.GSON.fromJson(new InputStreamReader(connection.getInputStream()), JsonObject.class);
