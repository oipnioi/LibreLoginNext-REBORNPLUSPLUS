...
package xyz.miguvt.miguloginnext.common.log;
...
public abstract class LogFilter {
    ...
    static {
        ...
        PROTECTED_COMMANDS.add("miguloginnext user register");
        PROTECTED_COMMANDS.add("miguloginnext user pass-change");
    }

    protected boolean checkMessage(String message) {
        // This sucks, but it's the only way to filter out the spam from the plugin
        if (message.contains("Plugin listener xyz.miguvt.miguloginnext.bungeecord.BungeeCordListener took")) return false;
