...
import xyz.miguvt.miguloginnext.paper.protocol.PacketListener;
...
public class PaperMiguLoginNext extends AuthenticMiguLoginNext<Player, World> {

    private final PaperBootstrap bootstrap;
    private PaperListeners listeners;
    private boolean started;

    public PaperMiguLoginNext(PaperBootstrap bootstrap) {
    ...
    protected void enable() {
        ...
        if (Bukkit.getOnlineMode()) {
            getLogger().error("!!!The server is running in online mode! MiguLoginNext won't start unless you set it to false!!!");
            disable();
            return;
        }
        ...
        if (isRunningUnderProxy()) {
            getLogger().error("!!!This server is running under a proxy, MiguLoginNext won't start!!!");
            getLogger().error("If you want to use MiguLoginNext under a proxy, place it on the proxy and remove it from the server.");
            disable();
            return;
        }
