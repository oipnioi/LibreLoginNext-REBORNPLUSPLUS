...
public class Blockers implements Listener {
    ...
    public Blockers(PaperMiguLoginNext plugin) {
    ...
    public void onTeleport(PlayerTeleportEvent event) {
        if (inLimbo(event.getPlayer())) {
            ...
            if (serverHandler.getLimboServers().contains(event.getTo().getWorld()) && !event.getPlayer().hasPermission("miguloginnext.limbo.access")) {
                event.setCancelled(true);
            }
        }
    }
